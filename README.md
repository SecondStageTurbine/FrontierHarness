...
- File context is bounded: up to 2,000 tree entries, 50 KB per context file, and 90 KB total file context. Every readable file still gets a content baseline, so a file too large to include in context can be modified while a file changed outside the run is never overwritten. Files the reader cannot open, such as binaries, cannot be modified at all. File preview supports UTF-8 text under 300 KB.
