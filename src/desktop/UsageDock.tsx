...
export function UsageDock(){
 ...
 return <aside className="usage-dock" aria-label="Subscription usage">
  <div className="usage-dock-head"><strong>Usage</strong><button className="icon-button" title="Refresh usage" aria-label="Refresh usage" disabled={limits.isFetching} onClick={()=>{setForce(true);setTimeout(()=>limits.refetch())}}><RefreshCw size={12} className={limits.isFetching?'spin':''}/></button></div>
