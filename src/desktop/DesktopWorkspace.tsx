...
import {Plus,Folder,FolderOpen,Search,MessageSquare,Settings2,PanelLeftClose,PanelRightOpen,Paperclip,ArrowUp,Square,LoaderCircle,X,ChevronDown,GitCompareArrows,Terminal,Code2,ListTree,ShieldCheck,ArrowRight,Eye,Pencil,Zap,ArrowRightLeft,Image as ImageIcon,Trash2} from 'lucide-react';
...
export default function DesktopWorkspace(){
 ...
 useEffect(()=>{const handler=(e:KeyboardEvent)=>{if((e.ctrlKey||e.metaKey)&&e.key==='n'){e.preventDefault();startNewSession()}if((e.ctrlKey||e.metaKey)&&e.key==='k'){e.preventDefault();document.getElementById('project-session-search')?.focus()}if(e.key==='Escape'){setProjectMenu(null);if(!settings&&!projectOpen)setPanel(null)}};window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler)},[project,settings,projectOpen]);
 ...
 async function send(){
  ...
  try{
   ...
   if(!current||newSession)current=await api.post<Session>(path(`/projects/${project.id}/sessions`),{name:'New session'});
    ...
    await api.post<Session>(path(`/projects/${project.id}/sessions/${current.id}/instructions`),{content:input,model_id:activeModelId,mode,attachment_ids:attachments.map(a=>a.id)});
