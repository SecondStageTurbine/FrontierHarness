...
export default function DesktopWorkspace(){
 const {tenant,tenants,switchTenant,path,notify}=useWorkspace();const refresh=useRefresh();
  ...
  const activity=useQuery({queryKey:['activity',tenant?.id],enabled:!!tenant,refetchInterval:4000,queryFn:async({signal})=>api.get<{active:{project_id:string;session_id:string}[]}>(path('/activity'),signal)});
  const activeProjects=new Set(activity.data?.active.map(a=>a.project_id)??[]);
