 ...
 return <Modal open={open} onClose={onClose} title={`${project.name} · settings`} description="What new sessions in this project start with, and what the agents are told." wide>
  <form onSubmit={save} className="project-settings">{error&&<p className="error-text" role="alert">{error}</p>}
   <div className="form-grid"><Field label="Default agent" hint="For new sessions; Adaptive picks per message."><select value={form.default_model_id} onChange={e=>setForm({...form,default_model_id:e.target.value})}><option value="">Adaptive (default)</option>{agents.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}<option value={ADAPTIVE}>Adaptive</option></select></Field>
    <Field label="Turn time limit (minutes)" hint="How long one agent may work on a turn before it is stopped. Every project gets at least 90 minutes; raise it for longer builds or engine test runs, up to 8 hours."><input type="number" min={90} max={480} value={form.turn_minutes} onChange={e=>setForm({...form,turn_minutes:e.target.value})} placeholder="90"/></Field>
   ...
   <h4 className="settings-subhead">Sandbox</h4>
   <p className="muted small">Frontier's own limits around every agent in this project, whatever posture a turn uses and whichever tool takes it.</p>
   <label className="toggle-row"><div><strong>Confine file changes to this folder</strong><p>The agent tool runs at Windows low integrity: it can change this project and its own settings folder, and nothing else you own, even under Full auto. Reading is not restricted. The first sandboxed turn labels the folder, which takes a moment in a large project.</p></div><input type="checkbox" role="switch" checked={form.sb_files} onChange={e=>setForm({...form,sb_files:e.target.checked})}/></label>
   <Field label="Network" hint="Tools and the package managers they run go through Frontier's filter, which refuses other hosts and lists them on the turn. A program built to ignore proxy settings is not stopped by this."><select value={form.sb_network} onChange={e=>setForm({...form,sb_network:e.target.value})}><option value="open">Open: no limit</option><option value="agent">Only the agent's own AI service</option><option value="allowlist">The agent's service and the hosts below</option></select></Field>
   {form.sb_network==='allowlist'&&<Field label="Allowed hosts" hint="Comma separated. *.example.com allows every subdomain."><textarea rows={2} value={form.sb_allow} onChange={e=>setForm({...form,sb_allow:e.target.value})} placeholder="registry.npmjs.org, pypi.org, files.pythonhosted.org, github.com, codeload.github.com, objects.githubusercontent.com"/>{!form.sb_allow.trim()&&<button type="button" className="linkish" onClick={()=>setForm({...form,sb_allow:'registry.npmjs.org, pypi.org, files.pythonhosted.org, github.com, codeload.github.com, objects.githubusercontent.com'})}>Add the common package registries and GitHub</button>}</Field>}
