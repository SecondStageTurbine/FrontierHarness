...
export function WorkspaceProvider({children}:{children:ReactNode}){
 ...
 return <Workspace.Provider value={{tenant,tenants,switchTenant,refreshTenants:()=>{void client.invalidateQueries({queryKey:['tenants']})},path:p=>`/t/${tenant?.id}${p}`,notify:setToast,theme,setTheme,accent,setAccent,advanced,setAdvanced,scale,setScale,spellcheck,setSpellcheck}}>{query.isPending||switchingTo!==null?<div className="loading-page">Opening your workspace…</div>:query.error?<div className="loading-page">{query.error.message}<button onClick={()=>query.refetch()}>Retry</button></div>:children}{toast&&<div role="status" className="toast">{toast}</div>}</Workspace.Provider>
}
