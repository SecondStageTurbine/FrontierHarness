...
export async function request<T>(path:string,options:RequestInit={}):Promise<T>{
  const response=await fetch(apiUrl(path),{...options,credentials:'same-origin',headers:{...(options.body instanceof FormData?{}:{'Content-Type':'application/json'}),...options.headers}});
...
export const api={get:<T,>(p:string,signal?:AbortSignal)=>request<T>(p,{signal}),post:<T,>(p:string,data?:unknown)=>request<T>(p,{method:'POST',body:JSON.stringify(data??{})}),put:<T,>(p:string,data:unknown)=>request<T>(p,{method:'PUT',body:JSON.stringify(data)}),patch:<T,>(p:string,data:unknown)=>request<T>(p,{method:'PATCH',body:JSON.stringify(data)}),delete:(p:string)=>request(p,{method:'DELETE'}),upload:<T,>(p:string,data:FormData)=>request<T>(p,{method:'POST',body:data})};
