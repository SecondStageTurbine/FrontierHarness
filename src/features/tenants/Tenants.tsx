...
export default function Tenants(){
 const q=useResource<Tenant[]>('/tenants'),models=useResource<Model[]>('/models');const {tenant,switchTenant,notify}=useWorkspace(),refresh=useRefresh();
 ...
 async function save(e:React.FormEvent){e.preventDefault();setBusy(true);setError(null);
  ...
  try{if(editing)await api.put(`/tenants/${editing.id}`,payload);else await api.post('/tenants',payload);setOpen(false);setForm(blank);notify('Workspace saved');await refresh()}
 ...
 <div className="model-grid">{q.data?.map(t=><article className="model-card" key={t.id}>
